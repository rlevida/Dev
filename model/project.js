...
var getDataCount = exports.getDataCount = ( tablename, data, advance , cb ) => {
    ...
    let query = `SELECT tb.typeId,Active,type,linkType,taskStatus.Issues,taskStatus.OnTrack  
                        FROM (select typeId,sum(IF(isActive="1",1,0)) as Active from project 
                        GROUP BY typeId) as tb 
                            LEFT JOIN type ON tb.typeId = type.id
                            LEFT JOIN (SELECT typeId,sum(Issues) as Issues,sum(OnTrack) as OnTrack FROM (SELECT typeId,projectId,IF(Issues>0,1,0) as Issues,IF(OnTrack>0,1,0) as OnTrack FROM project
                            LEFT JOIN (SELECT projectId,SUM(IF(dueDate>NOW(),1,0)) as OnTrack, SUM(IF(dueDate<NOW(),1,0)) as Issues FROM task) as tbTask ON project.id = tbTask.projectId) as tbpt GROUP BY typeId) as taskStatus ON tb.typeId = taskStatus.typeId;
`;
