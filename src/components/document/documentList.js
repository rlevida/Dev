...
class DocumentList extends React.Component {
    ...
    sortDocument(type) {
        ...
        if (document.New.length > 0) {
            const sortedDocument = _.orderBy(document.New, [`${type}`], [`${order == "asc" ? "desc" : "asc"}`]).map(e => {
                return e;
            });
            this.setState({
                ...this.state,
    ...
    viewDocument(data) {
        ...
        if (data.type !== "folder") {
            ...
            getData(
                `/api/document?isArchived=0&isActive=1&isDeleted=0&linkId=${projectId}&linkType=project&page=${1}&&userId=${loggedUser.data.id}&userType=${loggedUser.data.userType}&folderId=${data.id}&starredUser=${loggedUser.data.id}`,
                {},
                c => {
                    const { result, count } = { ...c.data };
                    if (c.status == 200) {
