...
class DocumentActionTab extends React.Component {
    ...
    handleFilter() {
        ...
        const projectId = match.params.projectId;
        const { uploadFrom, uploadTo, uploadedBy, tagWorkstream, tagTask, fileName, searchComment } = { ...document.Filter };
        const { ActiveTab } = { ...document };
        let requestUrl = `/api/document?&linkId=${projectId}&linkType=project&page=1&userId=${loggedUser.data.id}&userType=${loggedUser.data.userType}&starredUser=${loggedUser.data.id}&activeTab=${ActiveTab}`;
                        ...
                        <div class="flex">
                            ...
                            <div class="button-action">
                                {document.ActiveTab !== "activities" && document.Loading === "" && (
                                    <input class="form-control mr10" type="text" value={Filter.searchComment} onChange={e => this.handleSearchFilter("searchComment", e.target.value)} placeholder="Search comment" />
                                )}
