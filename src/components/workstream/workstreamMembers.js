                ...
                <div class={Loading == "RETRIEVING" ? "linear-background" : ""}>
                                    ...
                                    return (
                                        <tr key={index}>
                                            <td data-label="Username" class="td-left">
                                                <p class="m0">
                                                    <span>
                                                        <i class={`fa fa-circle mb0 mr5 ${session != "" && session != null ? "text-green" : "text-light-grey"}`} />
                                                    </span>
                                                    {username}
                                                </p>
                                            </td>
                                            ...
                                            <td data-label="Member Type">{memberType != "responsible" ? "Assigned to Task" : "Workstream Responsible"}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
                    ...
                    {currentPage != lastPage && Loading != "RETRIEVING" && (
                        <p class="mb0 text-center">
                            <a onClick={() => this.getNext()}>Load More Members</a>
                        </p>
                    )}
                    {members.List.length == 0 && Loading != "RETRIEVING" && (
                        <p class="text-center">
