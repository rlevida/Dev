...
export default class List extends React.Component {
    ...
    fetchData(page) {
        ...
        getData(
            `/api/document/getTaggedDocument?isDeleted=0&projectId=${project}&linkType=workstream&page=${page}&userId=${loggedUser.data.id}&userType=${loggedUser.data.userType}&workstreamId=${workstream.Selected.id}&tagType=document&starredUser=${
                loggedUser.data.id
            }`,
            {},
            c => {
    ...
    starredDocument({ id, isStarred, origin }) {
        ...
        postData(
            `/api/starred?projectId=${project}&document=${origin}`,
            {
                linkType: "document",
                linkId: id,
    ...
    sortDocument(type) {
        ...
        if (document.List.length > 0) {
            const sortedDocument = _.orderBy(document.List, [`${type}`], [`${order == "asc" ? "desc" : "asc"}`]).map(e => {
                return e;
            });
            this.setState({
                ...this.state,
                        ...
                        <table id="dataTable" class="table responsive-table table-bordered document-table mt30">
                            <tbody>
                                <tr>
                                    ...
                                    <th />
                                    <th>
                                        <a href="javascript:void(0)" onClick={() => this.sortDocument("origin")}>
                                            Name
                                        </a>
                                    </th>
                                    <th>
                                        <a href="javascript:void(0)" onClick={() => this.sortDocument("dateAdded")}>
                                            Uploaded date
                                        </a>
                                    </th>
                                    <th>Uploaded by</th>
                                    ...
                                    document.List.map((data, index) => {
                                        ...
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td class="new-document">
                                                    {" "}
                                                    <a href="javascript:void(0)" onClick={() => this.viewDocument(data)}>
                                                        <span class={data.type !== "folder" ? "glyphicon glyphicon-file" : "fa fa-folder"} />
                                                        {documentName}
                                                    </a>
                                                </td>
                                                ...
                                                <td>
                                                    {data.tags.length > 0 &&
                                                        data.tags.map((t, tIndex) => {
                                                            ...
                                                            return (
                                                                <span key={tIndex}>
                                                                    <label class="label label-primary" style={{ margin: "5px" }}>
                                                                        {t.label}
                                                                    </label>
                                                                    {tempCount > 16 && <br />}
                                                                </span>
                                                ...
                                                <td>
                                                    <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu  pull-right" aria-labelledby="dropdownMenu2">
                                                            <li>
                                                                <a href="javascript:void(0)" data-tip="Download" onClick={() => this.downloadDocument(data)}>
                                                                    Download
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a href="javascript:void(0)" data-tip="Rename" onClick={() => this.editDocument(data, "rename")}>
                                                                    Rename
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a href="javascript:void(0)" data-tip="Edit tags" onClick={() => this.editDocument(data, "tags")}>
                                                                    Edit Tags
                                                                </a>
                                                            </li>
                                                            <li>
                                                                <a href="javascript:void(0)" data-tip="Goto folder" onClick={() => this.goToFolder(data)}>
                                                                    Go to folder
                                                                </a>
                                                            </li>
                                                        </ul>
