...
export default class List extends React.Component {
    constructor(props) {
        ...
        _.map(["getNextResult", "setFilter", "fetchProject", "fetchType", "renderStatus", "getLateTasks", "getWorkstreams"], fn => {
    ...
    fetchProject(page) {
        ...
        getData(requestUrl, {}, c => {
            const projectList = [...project.List, ...c.data.result];
            dispatch({ type: "SET_PROJECT_LIST", list: projectList, page: page, hasNextPage: c.data.result.length >= 25 });
    ...
    getWorkstreams(id) {
        ...
        if (projectWorkstream.length > 0) {
            dispatch({
                type: "SET_WORKSTREAM_LIST",
                list: _.remove(workstream.List, listObj => {
        ...
        return (
            <tr key={index}>
                <td data-label="Project Name" class={entity_type == "project" ? "td-left" : ""}>
                    {entity_type == "project" ? (
                        <a class="text-violet" onClick={() => this.getWorkstreams(id)}>
                ...
                <td data-label="Type">
                    <p class={`m0 ${entity_type == "workstream" ? "text-italic" : ""}`}>
                        <span title={type}>
                            <i class={type == "Client" ? "fa fa-users" : type == "Private" ? "fa fa-lock" : type == "Internal" ? "fa fa-cloud" : type == "Output based" ? "fa fa-upload" : "fa fa-clock-o"} />
                        </span>
                    </p>
                </td>
                <td data-label="New Files">
                    <p class={`m0 ${entity_type == "workstream" ? "text-italic" : ""}`}>{new_files}</p>
                </td>
                <td data-label="Last Update">
                    <p class={`m0 ${entity_type == "workstream" ? "text-italic" : ""}`}>{date}</p>
                </td>
                <td data-label="Active Month Completion Rate">
                    <a data-tip data-for={`task-${index}`} onClick={() => this.getLateTasks({ id, type: entity_type })}>
                        ...
                        {((entity_type == "project" && workstream.length == 0) || entity_type == "workstream") && <p class={`m0 ${entity_type == "workstream" ? "text-italic" : ""}`}>{completionValue}%</p>}
                    </a>
                    <ReactTooltip id={`task-${index}`} aria-haspopup="true" type={"light"}>
                        <div class="wrapper">
                            <div class="display-flex mb5">
                                ...
                                <p class={`tooltip-label ${completion_rate.tasks_due_today.count > 0 ? "" : "text-light-grey"}`}>Task due today</p>
                            </div>
                            <div class="display-flex mb5">
                                <p class={`count ${completion_rate.tasks_for_approval.count > 0 ? "text-orange" : "text-light-grey"}`}>
                                    <strong>{completion_rate.tasks_for_approval.count}</strong>
                                </p>
                                <p class={`tooltip-label ${completion_rate.tasks_for_approval.count > 0 ? "" : "text-light-grey"}`}>Task for approval</p>
                            </div>
                            <div class="display-flex">
                                <p class={`count ${completion_rate.delayed_task.count > 0 ? "text-red" : "text-light-grey"}`}>
                                    <strong>{completion_rate.delayed_task.count}</strong>
                                </p>
                                <p class={`tooltip-label ${completion_rate.delayed_task.count > 0 ? "" : "text-light-grey"}`}>Delayed tasks</p>
                            </div>
                        </div>
                        <div class="sep" />
                        <div class="wrapper">
                            <div class="display-flex">
                                <p>
                                    Completed{" "}
                                    <strong>
                                        <span class="text-green">{completion_rate.completed.count}</span>
                                    </strong>{" "}
                                    out of <strong>{numberOfTasks}</strong> tasks
                                </p>
                            </div>
                        </div>
                    </ReactTooltip>
                    ...
                    {_.map(projectTypes, (projectType, index) => {
                        return (
                            <div class="flex-col" key={index}>
                                <a class={projectType.id == project.Filter.typeId ? "btn btn-default btn-active" : "btn btn-default"} onClick={() => this.setFilter("typeId", projectType.id)}>
                                    {projectType.name}
                                </a>
                            </div>
                        );
