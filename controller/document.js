            ...
            .then((nextThen, result) => {
                async.map(
                    ...
                    (e, mapCallback) => {
                        ...
                        Document.create(documentObj).then(res => {
                            async.parallel(
                                {
                                    ...
                                    activityLogsDocument: parallelCallback => {
                                        try {
                                            const logData = {
                                                ...
                                                old: "",
                                                new: res.origin,
                                                title: typeof isDuplicate !== "undefined" && isDuplicate ? (e.type === "document" ? "Document duplicated" : "Folder duplicated") : e.type === "document" ? "uploaded a document" : "created a folder",
                                                usersId: e.uploadedBy
                                            };

                                            ActivityLogsDocument.create(logData).then(c => {
                                    ...
                                    notification: parallelCallback => {
                                        if (res.dataValues.type !== "folder" && !isDuplicate && typeof data.tagWorkstream !== "undefined") {
                                            const workstreamIds = data.tagWorkstream.map(e => {
                                                return e.value;
                                            });
                                                        ...
                                                        .then(resNotification => {
                                                            Notification.findAll({
                                                                ...
                                                                .then(() => {
                                                                    async.map(
                                                                        notificationArr,
                                                                        ({ emailAddress, message, receiveEmail }, mapCallback) => {
                                                                            if (receiveEmail) {
                    ...
                    (err, mapCallbackResult) => {
                        nextThen(
                            mapCallbackResult.map(e => {
                                return e.documentIds;
                            }),
                            mapCallbackResult.map(e => {
                                return e.activityLogs;
                            })
                        );
                    }
                );
                ...
                try {
                    DocumentLink.findAll({
                        ...
                        .map(res => {
                            let resToReturn = {
                                ...res.document.toJSON(),
                                tagWorkstream: res.document.tagDocumentWorkstream.map(e => {
                                    return { value: e.tagWorkstream.id, label: e.tagWorkstream.workstream };
                                }),
                                tagTask: res.document.tagDocumentTask.map(e => {
                                    return { value: e.tagTask.id, label: e.tagTask.task };
                                }),
                            ...
                            try {
                                DocumentLink.findOne({
                                    ...
                                    let resToReturn = {
                                        ...
                                        tagWorkstream: findRes.document.tagDocumentWorkstream.map(e => {
                                            return { value: e.tagWorkstream.id, label: e.tagWorkstream.workstream };
                                        }),
                                        tagTask: findRes.document.tagDocumentTask.map(e => {
                                            return { value: e.tagTask.id, label: e.tagTask.task };
                                        }),
