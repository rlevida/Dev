...
exports.get = {
    index: async (req, cb) => {
        ...
        if (typeof queryString.userRole != "undefined" && queryString.userRole != "" && queryString.userRole > 3) {
            whereObj = {
                ...whereObj,
                [Sequelize.Op.or]: [
                    {
                ...
                async.parallel(
                    {
                        usersNotificationSetting: parallelCallback => {
                            UsersNotificationSetting.create({ usersId: result.id }).then(res => {
                                parallelCallback(null, res);
                            });
                        },
                        ...
                        teams: parallelCallback => {
                            if (typeof teams != "undefined") {
                                UsersTeam.destroy({ where: { usersId: result.id } }).then(res => {
                                    async.map(
                                        teams,
                                        (e, mapCallback) => {
                                            UsersTeam.create({ usersId: result.id, teamId: e.value }).then(createRes => {
                        ...
                        userRole: parallelCallback => {
                            if (typeof role != "undefined") {
                                UsersRole.destroy({ where: { usersId: result.id } }).then(res => {
                                    UsersRole.create({ usersId: result.id, roleId: role }).then(createRes => {
                        ...
                        project: parallelCallback => {
                            if (typeof project !== "undefined") {
                                Members.destroy({ where: { usersType: "users", userTypeLinkId: result.id, linkType: "project", memberType: "assignedTo" } }).then(res => {
                                    async.map(
                                        project,
                                        (e, mapCallback) => {
                                            try {
                                                Members.create({ usersType: "users", userTypeLinkId: result.id, linkType: "project", linkId: e.value, memberType: "assignedTo" }).then(createRes => {
                                                    mapCallback(null, createRes);
                                                });
                                            } catch (err) {
                                                mapCallback(err);
...
exports.put = {
                ...
                async.parallel(
                    {
                            ...
                            if (typeof teams !== "undefined") {
                                Teams.findAll({
                                    ...
                                    .then(o => {
                                        ...
                                        UsersTeam.destroy({ where: { usersId: body.id } }).then(res => {
                                            UsersTeam.bulkCreate(
                                                _.map(myTeam, o => {
                                                    return { usersId: body.id, teamId: o.value };
                                                })
                                            ).then(createRes => {
                                                parallelCallback(null, createRes);
                                            });
                        ...
                        userRole: parallelCallback => {
                            if (typeof role !== "undefined") {
                                UsersRole.destroy({ where: { usersId: body.id } }).then(res => {
                                    UsersRole.create({ usersId: body.id, roleId: role }).then(createRes => {
                                        parallelCallback(null, createRes);
                                    });
                                });
                            } else {
                        ...
                        project: parallelCallback => {
                            if (typeof project !== "undefined") {
                                Members.destroy({ where: { usersType: "users", userTypeLinkId: body.id, linkType: "project", memberType: "assignedTo" } }).then(res => {
                                    async.map(
                                        project,
                                        (e, mapCallback) => {
                                            try {
                                                Members.create({ usersType: "users", userTypeLinkId: body.id, linkType: "project", linkId: e.value, memberType: "assignedTo" }).then(createRes => {
                                                    mapCallback(null, createRes);
                                                });
                                            } catch (err) {
                                                mapCallback(err);
    ...
    changePassword: (req, cb) => {
        ...
        try {
            Users.update(data, { where: { id: id } }).then(res => {
    ...
    deleteUser: (req, cb) => {
        ...
        try {
            UsersRole.findAll({ where: { roleId: 1 } }).then(async res => {
                if (res.length <= 1 && res[0].usersId == id) {
                    ...
                    try {
                        ...
                        if (userMemberList.length > 0) {
                            ...
                            Users.update(body, { where: { id: id } }).then(updateRes => {
                                async.parallel(
                                    {
                                        role: parallelCallback => {
                                            UsersRole.update(body, { where: { usersId: id } }).then(userRoleRes => {
                                                parallelCallback(null, userRoleRes);
                                            });
                                        },
                                        team: parallelCallback => {
                                            UsersTeam.update(body, { where: { usersId: id } }).then(userTeamRes => {
